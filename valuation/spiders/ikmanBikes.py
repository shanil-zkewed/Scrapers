...
class IkmanSpider(scrapy.Spider):
    ...
    start_urls = ["https://ikman.lk/en/ads/sri-lanka/motorbikes-scooters"]
    ...
    def parse(self, response):
        ...
        for i in range(1,40):
            next_page = "https://ikman.lk/en/ads/sri-lanka/motorbikes-scooters?sort=date&order=desc&buy_now=0&urgent=0&page="+str(i)      
    ...
    def parse_veh_page(self, response):
        ...
        carItem["category"] = response.css('.ad-meta-desktop--1Zyra span::text')[0].get(),
        carItem["make"] = response.css('.ad-meta-desktop--1Zyra span::text')[2].get(),
        carItem["model"] = response.css('.ad-meta-desktop--1Zyra span::text')[3].get(),
        carItem["year" ] = response.css('.ad-meta-desktop--1Zyra span::text')[4].get(),
        carItem["price"] = response.css('.amount--3NTpl::text').get(),
        carItem["mileage"] = response.css('.word-break--2nyVq.value--1lKHt::text')[2].get(),
        carItem["engine_capability"] = response.css('.word-break--2nyVq.value--1lKHt::text')[1].get(),
        carItem["location"] = response.css('span.sub-title--37mkY span::text')[2].get(),
        carItem["date"] = response.css('span.sub-title--37mkY div::text')[2].get(),
